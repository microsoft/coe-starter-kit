# CoE Starter Kit Documentation

This directory contains additional documentation and FAQs for the Center of Excellence (CoE) Starter Kit.

## Official Documentation

The primary documentation for the CoE Starter Kit is hosted on Microsoft Learn:
- **Main Documentation**: https://learn.microsoft.com/power-platform/guidance/coe/starter-kit

## Additional Resources in This Repository

### Frequently Asked Questions (FAQs)

- **[FAQ: Licensing Display in CoE](FAQ-Licensing-Display.md)** - Information about tracking Power Platform licenses and capacity in the CoE Starter Kit

### Issue Response Templates

- **[GitHub Issue Comment: Licensing](GITHUB-ISSUE-COMMENT.md)** - Template response for licensing-related questions
- **[Issue Response: Licensing](ISSUE-RESPONSE-Licensing.md)** - Detailed response document for licensing questions

## Contributing

If you have suggestions for additional documentation or have found an issue, please:
1. Check existing [issues](https://github.com/microsoft/coe-starter-kit/issues) first
2. Submit a new issue or pull request following our [contribution guidelines](../HOW_TO_CONTRIBUTE.md)

## Related Resources

- [CoE Starter Kit Setup Guide](https://learn.microsoft.com/power-platform/guidance/coe/setup)
- [CoE Starter Kit Components](https://learn.microsoft.com/power-platform/guidance/coe/starter-kit-explained)
- [Power BI Dashboard Setup](https://learn.microsoft.com/power-platform/guidance/coe/setup-powerbi)
- [GitHub Issues & Backlog](https://github.com/microsoft/coe-starter-kit/issues)
